...
export function VisionMission() {
  return (
    <section aria-label="비전과 미션" className="px-6 py-24 lg:px-10 lg:py-32">
      <div className="mx-auto flex max-w-[1200px] flex-col gap-24">
        {blocks.map((block) => (
          <article key={block.eyebrow} className="flex flex-col items-center text-center">
            <p className="text-[15px] font-bold tracking-[0.1em] text-foreground">{block.eyebrow}</p>
            <div aria-hidden="true" className="mt-4 h-px w-16 bg-primary" />

            <h3 className="mt-14 text-[44px] font-bold leading-tight text-primary md:text-[56px] lg:text-[64px]">
              {block.title}
            </h3>

            {block.subtitle && (
              <p className="mt-10 text-balance text-[16px] font-semibold text-foreground md:text-[18px]">
                {block.subtitle}
              </p>
            )}

            <p
              className={`text-pretty text-[13px] leading-relaxed text-muted-foreground md:text-sm ${
                block.subtitle ? "mt-10" : "mt-10"
              }`}
            >
              {block.description}
            </p>
